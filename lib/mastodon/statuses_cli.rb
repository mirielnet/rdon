...
module Mastodon
  class StatusesCLI < Thor
    ...
    long_desc <<~LONG_DESC
      ...
      With the --vacuum option, Reduce the file size and update the statistics.
      This option locks the table for a long time, so run it offline.
    LONG_DESC
    def remove
      ...
      ActiveRecord::Base.connection.exec_insert(<<-SQL.squish, 'SQL', [[nil, max_id]])
        ...
        AND NOT EXISTS (SELECT 1 FROM statuses AS statuses1 WHERE statuses.id = statuses1.quote_id)

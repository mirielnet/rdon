...
class Scheduler::FollowRecommendationsScheduler
  ...
  def perform
    fallback_recommendations = FollowRecommendation.safe.limit(100).to_a

    I18n.available_locales.each do |locale|
      recommendations = []

      # We can skip the work if no accounts with that language exist
      recommendations.concat(FollowRecommendation.safe.localized(locale).limit(100)) if AccountSummary.safe.localized(locale).exists?

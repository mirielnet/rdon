...
class Api::V1::Accounts::FollowerAccountsController < Api::BaseController
  ...
  def hide_results?
    @account.suspended? || (@account.hide_followers? && current_account&.id != @account.id) || (@account&.user&.setting_hide_followers_from_yourself && current_account&.id == @account.id) || (current_account && @account.blocking?(current_account))
  end

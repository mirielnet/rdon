...
const mapDispatchToProps = (dispatch, { listId }) => ({
  ...
  onSubscribe (account) {
    if (account.getIn(['relationship', 'subscribing', listId], new ImmutableMap).size > 0) {
...
class List extends ImmutablePureComponent {
  ...
  render () {
    ...
    const subscribing = account.getIn(['relationship', 'subscribing', list.get('id')], new ImmutableMap).size > 0;

...
const getOrderedTags = createSelector([state => state.get('favourite_tags')], favourite_tags => {
  ...
  return favourite_tags.toList().filter(item => !!item).sort((a, b) => a.get('updated_at') < b.get('updated_at')).take(10);
});
    ...
    return (
      <div>
        ...
        {favourite_tags.map(favourite_tag => (
          <NavLink key={favourite_tag.get('name')} className='column-link column-link--transparent' strict to={`/timelines/tag/${favourite_tag.get('name')}`}><Icon className='column-link__icon' id='hashtag' fixedWidth />{favourite_tag.get('name')}</NavLink>
        ))}
      </div>
    );
  }

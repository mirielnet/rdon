...
export function subscribeAccount(id, reblogs = true, list_id = null) {
  return (dispatch, getState) => {
    const alreadySubscribe = (list_id ? getState().getIn(['relationships', id, 'subscribing', list_id], new ImmutableMap) : getState().getIn(['relationships', id, 'subscribing'], new ImmutableMap)).size > 0;
    const locked = getState().getIn(['accounts', id, 'locked'], false);
